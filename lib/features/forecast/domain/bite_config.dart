...
class BiteConfig {
  ...
  // ── Нерест ───────────────────────────────────────────────────
  // Нерест триггерится УСТОЙЧИВЫМ прогревом воды в видовую полосу температур
  // внутри сезонного окна — это и есть единственный честный сигнал, который мы
  // можем дать (точную дату оценка воды как EMA воздуха не позволяет). См.
  // [SpawnAdvisor]: полоса + календарное окно + знак тренда воды → фаза.
  ...
  /// Месяцы (1..12, для СЕВЕРНОГО полушария) календарного окна нереста. На юге
  /// сдвигаются на полгода тем же приёмом, что и [seasonByMonth].
  final Set<int> spawnMonths;
  ...
  static const BiteConfig carp = BiteConfig(
    ...
    // Нерест карпа: устойчивый прогрев до ~17–20 °C, конец весны — июнь.
    spawnTempLow: 17,
    spawnTempHigh: 20,
    spawnMonths: {4, 5, 6},
  );
  ...
  static const BiteConfig crucian = BiteConfig(
    ...
    // Нерест карася позже и теплее карпа: ~14–18 °C, май–июль, часто волнами.
    spawnTempLow: 14,
    spawnTempHigh: 18,
    spawnMonths: {5, 6, 7},
  );
