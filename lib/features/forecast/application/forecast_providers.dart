...
/// Проточная и мелкая вода идёт за воздухом быстро, крупные озёра — медленнее.
/// Когда водоём неизвестен — дефолт клиента.
///
/// Потолок τ для крупной воды намеренно умеренный (≤15 сут). Сверка EMA с
/// ИЗМЕРЕННОЙ температурой реальных озёр (Monona, Walker, Boundary, датчики
/// USGS) показала: прежние τ=20–30 сильно переглаживали (RMSE 3–5 °C), тогда
/// как τ≈12–15 даёт RMSE ~2–3 °C на всём диапазоне размеров. Причина — площадь
/// не отражает глубину, а летом поверхность гонит тонкий эпилимнион (~2–4 м),
/// а не весь столб воды, поэтому «большая площадь ⇒ большая инерция» завышает τ.
double waterTauForBody(WaterBody? body) {
  ...
  switch (body.type) {
    ...
    case WaterBodyType.other:
      ...
      if (ha < 10) return 6;
      if (ha < 100) return 10;
      if (ha < 1000) return 12;
      return 15; // крупное озеро/вдхр: умеренная инерция (не 30 — переглаживало)
  }
}
