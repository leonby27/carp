...
///     осенью теплее);
///  2) достаточная ПРЕДЫСТОРИЯ воздуха (≥ ~3·τ для самой большой воды), чтобы
///     EMA успела «накопить» сезонное тепло до начала прогноза — ради этого и
///     тянем [_pastDays] назад.
class OpenMeteoClient {
  ...
  /// Вклад сида EMA затухает как exp(−D/τ), поэтому для сходимости к реальному
  /// сезонному уровню нужно D ≳ 3·τ (при 4·τ остаточное влияние сида <2%). τ
  /// задаётся типом водоёма из OSM и теперь доходит до 15 сут (см.
  /// `waterTauForBody`), поэтому окна 60 = 4·15 сут с запасом хватает, чтобы
  /// даже самая инертная вода успела разогреться. Водоём грузится ПАРАЛЛЕЛЬНО с
  /// погодой, так что τ на момент запроса ещё неизвестна — берём окно под
  /// worst-case τ для всех.
  static const _pastDays = 60;
